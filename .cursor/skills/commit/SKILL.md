...
- `docs/local-template-apigee.yaml` — **local-only, gitignored** (`docs/local-*`) OpenAPI draft for publishing the
  ...
  against `GET /docs/integration-json`, keeping the corporate header (`info`/`servers`/`x-*`)
  untouched. It can never be committed — after updating, republish it on the Apigee portal.

**Root:**

- `README.md` — setup, install, scripts/commands, env vars, run/deploy steps, CI & security gates, project structure.

**AI guides (keep BOTH mirrors in sync):**

- `CLAUDE.md` (root) and `.cursor/rules/project-overview.mdc` — monorepo overview, top-level commands, commit rules.
- `apps/backend/CLAUDE.md` and `.cursor/rules/database-migrations.mdc` — Prisma schema/migrations protocol, DB environment parity, hosting notes.
- `apps/frontend/CLAUDE.md` and `.cursor/rules/frontend-design-ux.mdc` — design system & tokens, UX/accessibility conventions, auth/session helpers, PII display rules, tone of voice.
- These pairs mirror each other; a change that makes one stale makes its pair stale too. Update both sides in the same commit.
...
1. **Prisma schema drift:** if `apps/backend/prisma/schema.prisma` or `apps/backend/prisma/migrations/**`
   ...
   "up to date") and that the Prisma client is regenerated. See `apps/backend/CLAUDE.md`
   (mirrored in `.cursor/rules/database-migrations.mdc`) for the full protocol.
...
Do NOT `git push` — the user pushes (push to `master` auto-deploys Render).
