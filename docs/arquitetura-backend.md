...
- **Escopo do token do painel:** além do estado de exibição, com o dia aberto ele obtém o token de entrada mostrado no QR da TV (`qrEntry`) — conteúdo feito para exibição pública. Consequência operacional: se o token do painel vazar, **revogue/rotacione o token do painel** na administração (não basta rotacionar o QR)
